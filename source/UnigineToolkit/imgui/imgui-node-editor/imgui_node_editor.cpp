...
// static void ImDrawList_TransformChannel_Inner(ImVector<ImDrawVert>& vtxBuffer, const
// ImVector<ImDrawIdx>& idxBuffer, const ImVector<ImDrawCmd>& cmdBuffer, const ImVec2& preOffset,
// const ImVec2& scale, const ImVec2& postOffset)
//{
...
//         }
//
//         for (auto vtx = vtxBuffer.Data + minIndex, vtxEnd = vtxBuffer.Data + maxIndex + 1; vtx <
//         vtxEnd; ++vtx)
//         {
...
// static void ImDrawList_TransformChannels(ImDrawList* drawList, int begin, int end, const ImVec2&
// preOffset, const ImVec2& scale, const ImVec2& postOffset)
//{
...
//     {
//         ImDrawList_TransformChannel_Inner(vtxBuffer, drawList->IdxBuffer, drawList->CmdBuffer,
//         preOffset, scale, postOffset);
//         ++begin;
//     }
//
...
//         ImDrawList_TransformChannel_Inner(vtxBuffer, channel.IdxBuffer, channel.CmdBuffer,
//         preOffset, scale, postOffset);
//     }
//
//     if (lastCurrentChannel != 0)
...
// static void ImDrawList_ClampClipRects_Inner(ImVector<ImDrawCmd>& cmdBuffer, const ImVec4&
// clipRect, const ImVec2& offset)
//{
//     for (auto& cmd : cmdBuffer)
//     {
...
// static void ImDrawList_TranslateAndClampClipRects(ImDrawList* drawList, int begin, int end, const
// ImVec2& offset)
//{
//     int lastCurrentChannel = drawList->_ChannelsCurrent;
//     if (lastCurrentChannel != 0)
...
static void ImDrawList_PathBezierOffset(ImDrawList *drawList, float offset, const ImVec2 &p0,
...
/*
static void ImDrawList_PolyFillScanFlood(ImDrawList *draw, std::vector<ImVec2>* poly, ImColor color,
int gap = 1, float strokeWidth = 1.0f)
{
...
return;
	...
	while (y < max.y)
	{
...

		{
			...
			sort(scanHits.begin(), scanHits.end(), [](ImVec2 const &a, ImVec2 const &b) { return a.x
< b.x; });
...
static void ImDrawList_AddBezierWithArrows(ImDrawList *drawList, const ImCubicBezierPoints &curve,
	float thickness, float startArrowSize, float startArrowWidth, float endArrowSize,
	...
	else
	{
		if (startArrowSize > 0.0f)
		{
			const auto start_dir =
				ImNormalized(ImCubicBezierTangent(curve.P0, curve.P1, curve.P2, curve.P3, 0.0f));
			const auto start_n = ImVec2(-start_dir.y, start_dir.x);
			const auto half_width = startArrowWidth * 0.5f;
			const auto tip = curve.P0 - start_dir * startArrowSize;
		...
		{
			const auto end_dir =
				ImNormalized(ImCubicBezierTangent(curve.P0, curve.P1, curve.P2, curve.P3, 1.0f));
			const auto end_n = ImVec2(-end_dir.y, end_dir.x);
			const auto half_width = endArrowWidth * 0.5f;
			const auto tip = curve.P3 + end_dir * endArrowSize;
...
void ed::Pin::Draw(ImDrawList *drawList, DrawFlags flags)
{
	if (flags & Hovered)
	{
		...
		if (m_BorderWidth > 0.0f)
		{
			...
			drawList->AddRect(
				m_Bounds.Min, m_Bounds.Max, m_BorderColor, m_Rounding, m_Corners, m_BorderWidth);
		}
	...
	if (flags == Detail::Object::None)
	{
		...
		if (IsGroup(this))
		{
			...
			if (m_GroupBorderWidth > 0.0f)
			{
				...
				drawList->AddRect(m_GroupBounds.Min, m_GroupBounds.Max, m_GroupBorderColor,
...
ImRect ed::Node::GetRegionBounds(NodeRegion region) const
{
	...
	else if (m_Type == NodeType::Group)
	{
		const float activeAreaMinimumSize =
			ImMax(ImMax(Editor->GetView().InvScale * c_GroupSelectThickness, m_GroupBorderWidth),
...
ed::NodeRegion ed::Node::GetRegion(const ImVec2 &point) const
{
	...
	else if (m_Type == NodeType::Group)
	{
		static const NodeRegion c_Regions[] = {// Corners first, they may overlap other regions.
			...
			NodeRegion::BottomRight, NodeRegion::Header, NodeRegion::Top, NodeRegion::Bottom,
			NodeRegion::Left, NodeRegion::Right, NodeRegion::Center};
...
void ed::Link::Draw(ImDrawList *drawList, ImU32 color, float extraThickness) const
{
	...
	ImDrawList_AddBezierWithArrows(drawList, curve, m_Thickness + extraThickness,
		m_StartPin && m_StartPin->m_ArrowSize > 0.0f ? m_StartPin->m_ArrowSize + extraThickness
													 : 0.0f,
		m_StartPin && m_StartPin->m_ArrowWidth > 0.0f ? m_StartPin->m_ArrowWidth + extraThickness
													  : 0.0f,
		...
		true, color, 1.0f, m_StartPin && m_StartPin->m_SnapLinkToDir ? &m_StartPin->m_Dir : nullptr,
...
void ed::EditorContext::End()
{
	...
	auto control = BuildControl(
		m_CurrentAction && m_CurrentAction->IsDragging());	  // NavigateAction.IsMovingOverEdge()
	...
	if (sortGroups
		|| ((m_Settings.m_DirtyReason & (SaveReasonFlags::Position | SaveReasonFlags::Size))
	...
	// node drawing order.
	{
		...
		auto groupsItEnd =
			std::find_if(m_Nodes.begin(), m_Nodes.end(), [](Node *node) { return !IsGroup(node); });
...
#if 1	 // #FIXME
	{
		...
		ImU32 GRID_COLOR = GetColor(
			StyleColor_Grid, ImClamp(m_Canvas.ViewScale() * m_Canvas.ViewScale(), 0.0f, 1.0f));
	...
	// #debug
	// drawList->AddRectFilled(ImVec2(-10.0f, -10.0f), ImVec2(10.0f, 10.0f), IM_COL32(255, 0, 255,
	// 255));
...
int ed::EditorContext::CountLiveNodes() const
{
	return (int)std::count_if(
		m_Nodes.begin(), m_Nodes.end(), [](const Node *node) { return node->m_IsLive; });
}
...
{
	return (int)std::count_if(
		m_Links.begin(), m_Links.end(), [](const Link *link) { return link->m_IsLive; });
}
...
ed::Control ed::EditorContext::BuildControl(bool allowOffscreen)
{
	...
	const auto widgetHovered =
		ImGui::IsMouseHoveringRect(m_Canvas.ViewRect().Min, m_Canvas.ViewRect().Max, true);
	...
	auto emitInteractiveAreaEx = [&activeId](ObjectId id, const ImRect &rect,
									 ImGuiButtonFlags extraFlags) -> int {
	...
	auto checkInteractionsInArea = [this, &emitInteractiveArea, &hotObject, &activeObject,
									   &clickedObject, &doubleClickedObject](
	...
	return Control(hotObject, activeObject, clickedObject, doubleClickedObject, isBackgroundHot,
		isBackgroundActive, backgroundClickButonIndex, backgroundDoubleClickButtonIndex);
}
...
void ed::EditorContext::ShowMetrics(const Control &control)
{
	...
	ImGui::Text("Mouse: { x=%.0f y=%.0f } global: { x=%g y=%g }", localMousePos.x, localMousePos.y,
	...
	{
		ImGui::SameLine();
		ImGui::Text("{ x=%g y=%g w=%g h=%g }", node->m_Bounds.Min.x, node->m_Bounds.Min.y,
	...
	ImGui::Text(
		"Action Is Dragging: %s", m_CurrentAction && m_CurrentAction->IsDragging() ? "Yes" : "No");
...
bool ed::Settings::Parse(const std::string &string, Settings &settings)
{
	...
	if (viewValue.is_object())
	{
		...
		result.m_ViewZoom =
			viewZoomValue.is_number() ? static_cast<float>(viewZoomValue.get<double>()) : 1.0f;
...
//
//------------------------------------------------------------------------------
const float ed::NavigateAction::s_DefaultZoomLevels[] = {0.1f, 0.15f, 0.20f, 0.25f, 0.33f, 0.5f,
	0.75f, 1.0f, 1.25f, 1.50f, 2.0f, 2.5f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f};
...
ed::EditorAction::AcceptResult ed::NavigateAction::Accept(const Control &control)
{
	...
	if (Editor->CanAcceptUserInput() && ImGui::IsKeyPressed(ImGuiKey_F)
		&& Editor->AreShortcutsEnabled())
	{
		const auto zoomMode =
			io.KeyShift ? NavigateAction::ZoomMode::WithMargin : NavigateAction::ZoomMode::None;
		...
		if (!Editor->GetSelectedObjects().empty())
		{
				...
				|| (zoomMode != NavigateAction::ZoomMode::None))
			{
				m_LastSelectionId = Editor->GetSelectionId();
				NavigateTo(
					Editor->GetSelectionBounds(), zoomMode, -1.0f, NavigationReason::Selection);
			}
			else
...
void ed::NavigateAction::ShowMetrics()
{
	...
	ImGui::Text("    Visible Rect: { l=%g t=%g, r=%g b=%g w=%g h=%g }", m_VisibleRect.Min.x,
		m_VisibleRect.Min.y, m_VisibleRect.Max.x, m_VisibleRect.Max.y,
		m_VisibleRect.Max.x - m_VisibleRect.Min.x, m_VisibleRect.Max.y - m_VisibleRect.Min.y);
}
...
void ed::SizeAction::ShowMetrics()
{
	...
	{
		ImGui::Text("    Bounds: { x=%g y=%g w=%g h=%g }", m_SizedNode->m_Bounds.Min.x,
		...
		ImGui::Text("    Start Bounds: { x=%g y=%g w=%g h=%g }", m_StartBounds.Min.x,
...
bool ed::DragAction::Process(const Control &control)
{
	...
	if (control.ActiveObject == m_DraggedObject)
	{
		...
		if (auto draggedNode = m_DraggedObject->AsNode())
		{
			...
			auto testPivot = [this, &x, &y, &draggedOrigin, &dragOffset, &alignPivot](
								 const ImVec2 &pivot) {
		...
		auto alignedOffset = Editor->AlignPointToGrid(draggedOrigin + dragOffset + alignPivot)
...
bool ed::SelectAction::Process(const Control &control)
{
	...
	if (ImGui::IsMouseDragging(Editor->GetConfig().SelectButtonIndex, 0))
	{
		...
		auto topLeft =
			ImVec2(std::min(m_StartPoint.x, m_EndPoint.x), std::min(m_StartPoint.y, m_EndPoint.y));
		auto bottomRight =
			ImVec2(ImMax(m_StartPoint.x, m_EndPoint.x), ImMax(m_StartPoint.y, m_EndPoint.y));
...
void ed::SelectAction::Draw(ImDrawList *drawList)
{
	...
	const auto fillColor =
		Editor->GetColor(m_SelectLinkMode ? StyleColor_LinkSelRect : StyleColor_NodeSelRect, alpha);
...
ed::EditorAction::AcceptResult ed::ShortcutAction::Accept(const Control &control)
{
	...
	if (candidateAction != None)
	{
		if (candidateAction != Paste && candidateAction != CreateNode)
		{
			...
			// const auto hasOnlyLinks = std::all_of(Context.begin(), Context.end(), [](Object*
			// object) { return object->AsLink() != nullptr; }); if (hasOnlyLinks)
			//    return False;
...
void ed::DeleteItemsAction::ShowMetrics()
{
	...
	ImGui::Text("%s:", GetName());
	ImGui::Text("    Active: %s", m_IsActive ? "yes" : "no");
	// ImGui::Text("    Node: %s (%d)", getObjectName(DeleteItemsgedNode), DeleteItemsgedNode ?
	// DeleteItemsgedNode->ID : 0);
}
...
void ed::NodeBuilder::Begin(NodeId nodeId)
{
	...
	{
		ImGui::SetCursorPos(
			ImGui::GetCursorPos() + ImVec2(editorStyle.NodePadding.x, editorStyle.NodePadding.y));
		ImGui::BeginGroup();
	}
}
...
void ed::NodeBuilder::BeginPin(PinId pinId, PinKind kind)
{
	...
	m_CurrentPin->m_Dir =
		kind == PinKind::Output ? editorStyle.SourceDirection : editorStyle.TargetDirection;
...
void ed::NodeBuilder::EndPin()
{
	...
	// Editor->GetDrawList()->AddRect(m_CurrentPin->m_Bounds.Min, m_CurrentPin->m_Bounds.Max,
	// IM_COL32(255, 255, 0, 255));

	// #debug: Draw pin pivot rectangle
	// Editor->GetDrawList()->AddRect(m_CurrentPin->m_Pivot.Min, m_CurrentPin->m_Pivot.Max,
