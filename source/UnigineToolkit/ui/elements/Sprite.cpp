...
void Sprite::init()
{
	...
	// create
	sprite_w = WidgetSprite::create(get_gui(), "white.texture");
	sprite_w->arrange();
...
void Sprite::applyPropertyChanges()
{
...
void Sprite::setTexture(const char *texture_path)
{
...
void Sprite::setColor(const Unigine::Math::vec4 &in_color)
{
	color = in_color;
	sprite_w->setColor(in_color);
}

void Sprite::setFixedRatio(bool in_fixed_ratio)
{
...
void Sprite::setRotation(float angle_deg)
{
	if (angle == angle_deg)
		return;
	angle = angle_deg;
...
bool Sprite::isHover(int x, int y) const
{
...
bool Sprite::isHover(int x0, int y0, int x1, int y1) const
{
	if (compare(angle, 0.f))
...
void Sprite::on_enable()
{
	Element::on_enable();
	if (sprite_w)
	{
		if (!sprite_w->getRender())
			setTexture(texture_file);
		sprite_w->setHidden(false);
	}
}

void Sprite::on_disable()
{
	if (sprite_w)
	{
		if (!sprite_w->getRender())
...
void Sprite::apply_order_to_widgets()
{
	if (!sprite_w)
		return;
...
void Sprite::set_gui(const Unigine::GuiPtr &gui)
{
	if (!sprite_w)
		return;
