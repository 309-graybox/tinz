...
{
	Engine::get()->addPlugin("RyutpEnhancedInput");

	auto ei = EISystem::get();
	if (!ei)
	{
		Log::error("Can not load RyutpEnhancedInput\n");
		return 0;
	}

	Log::message("RyutpEnhancedInput was loaded\n");

	// initialization for c++ component system
	ComponentSystem::get()->initialize();

    World::loadWorld("Sacrifillion/scene/main_menu");
