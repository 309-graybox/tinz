...
PlayerStateWalk::PlayerStateWalk(PlayerContext &ctx, const char *actionName)
	: _move()
{
	...
	auto l = ms->addLayer();
	setAnimationLayer(l);
	ms->setLayerAnimationFilePath(l, "character/animations/Walking.anim");
	ms->setLayer(l, true, 0.0f);
}

float PlayerStateWalk::getScore() const
{
